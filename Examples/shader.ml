...
object
  ...
  let storm_blink =
    ...
    let create_point x =
      ...
      Vertex.create ~position ~color ()
    in
    try 
object
  inherit effect "storm + blink"
  val myPoints = new vertex_array ~primitive_type:PrimitiveType.Points (List.map create_point (range 0 40000))
...
object (self)
  ...
  method private on_load =
	...
	let texture_rect = IntRect.({ left = 96 * i; top = 0 ; width = 96 ; height = 96 }) in
